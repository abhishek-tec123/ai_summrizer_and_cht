...
def upload_file():
    ...
    try:
        ...
        with ThreadPoolExecutor(10) as executor:
            future_to_chunk = {executor.submit(summarize_chunk, llm, chunk): chunk for chunk in chunks}
            for i, future in enumerate(as_completed(future_to_chunk), start=1):
                ...
                try:
                    ...
                    if summary:
                        summaries.append(summary)
                        logging.info(f"Chunk {i}/{total_chunks} summarized successfully.")
                
                    else:
                        logging.warning(f"Chunk {i} could not be summarized. Skipping.")
                    # time.sleep(1)
                except Exception as e:
